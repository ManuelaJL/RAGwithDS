...
def removeLLMsPreamble(text):
    ...
    if lines[0] and "summary" in lines[0]:
...
def summarizePage(text: str):
    ...
    chain = prompt | llm
    summaryFromLLM = chain.invoke({"context": text})
...
def summarizeDocument(text: str):
    ...
    Ignore mentions of moodle, illustrative examples, citations, or rhetorical questions unless they are central to the argument.\\n\\n{context}
    Output:
    [SUMMARY]
    <your summary here>
    """
...
def summarizeAndSaveEachFile_fromPageSummaries(summaries_of_Pages):
    for path, pageSums in summaries_of_Pages.items():
        ...
        filename = os.path.basename(path) + "_byPagesThenOverall.txt"
...
def summarizeAndSaveEachFile_fromFullContent(fullTexts):
    for path, content in fullTexts.items():
        ...
        folder_to_save = getSummaryOutputFolder(path)
        os.makedirs(folder_to_save, exist_ok=True)
        filename = os.path.basename(path) + "_fromWholeContent.txt"
        full_path = os.path.join(folder_to_save, filename)
        with open(full_path, "w", encoding="utf-8") as file:
            file.write(docSum)
        print(f"\nSaved: {full_path}")
...
def summarizeFirstByPageThenByDocumentAndSave():
...
if __name__ == "__main__":
    ...
    # folder_to_summarize = r'C:\Data Science\MAS Data Science\2019FS Datenanalyse\ExpVis'
